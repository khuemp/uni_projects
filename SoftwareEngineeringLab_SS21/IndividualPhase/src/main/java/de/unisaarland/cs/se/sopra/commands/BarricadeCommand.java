...
import de.unisaarland.cs.se.sopra.abilities.BarricadeVisitor;
import de.unisaarland.cs.se.sopra.model.Model;
import de.unisaarland.cs.se.sopra.model.Survivor;
import java.util.Optional;

public class BarricadeCommand extends InGameCommand {

    private final int survivorId;
    private final int entrance;

    public BarricadeCommand(final int commId, final int survivorId, final int entrance) {
        super(commId);
    ...
    public void run(final Model model, final ConnectionWrapper connection) {
        ...
        if (model.hasSurvivor(playerId, survivorId)) {
            ...
            if (optionalSurvivor.isPresent()) {
                ...
                final BarricadeVisitor visitor = new BarricadeVisitor(model, connection,
